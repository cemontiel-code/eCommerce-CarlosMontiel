  ...
  return (
    <div className="card  lg:card-side rounded-2xl w-11/12 mx-auto h-full shadow-xl bg-primary-content bg-opacity-50 backdrop-blur-lg flex flex-col">
      <Link to="/" className="group btn btn-sm absolute txt-lg z-40">
        <span className="hidden  group-hover:block  ">volver a homapage</span> 
        <FontAwesomeIcon icon={faAnglesLeft} />
      </Link>

      <figure className="md:w-[40%] ">
        <img
          className="px-2  mb-auto mt-auto md:mr-auto rounded-2xl"
          src={picURL}
          alt={`${category}, ${id} `}
        />
      </figure>
      ...
      <div className="card-body">
        <div className="text-sm breadcrumbs ">
          <ul>
            ...
            <li>
              <Link to={categoryIdString} className="link">
                {category}
              </Link>
            </li>
            <li>{title}</li>
